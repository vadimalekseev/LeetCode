 ...
 */
function buildGithubUrl(pathToFile) {
...
function generateTableBody(problems) {
  return problems.map((problem, idx) => `|${idx + 1}|[${problem.title}](${buildLeetcodeUrl(problem.id)})|${problem.difficulty}|${problem.solvings.map((solut) => `[${solut.title}](${solut.path})`).join(", ")}|`).join(EOL);
}

/**
 *
 * @param {string} summary Table header.
 * @param {(item: problems[0]) => boolean} [difficultyPredicate] For filter problems.
 * @returns The table.
 */
function generateSolvingsTable(summary, difficultyPredicate) {
  return `<details>
<summary>${summary}</summary>

| #     | Problem            | Difficulty | Solutions       |
|:-----:|:------------------:|:----------:|:---------------:|
${difficultyPredicate ? generateTableBody(problems.filter(difficultyPredicate)) : generateTableBody(problems)}
...
let mdFile = `# This repo contains my solvings leetcode problems

This file was generated automatically by [buildMdFile.js](${buildGithubUrl("scripts/buildMdFile.js")}) script.

Count of all problems solved: ${problems.length}

${Object.keys(difficultyAmounts).map(key => `Count of solved problems with difficulty "${key}": ${difficultyAmounts[key]}`).join(EOL + EOL)}
...
fs.writeFile(path.join(__dirname, "..", "README.md"), mdFile);
